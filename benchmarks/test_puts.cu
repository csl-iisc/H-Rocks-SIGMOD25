...
int main(int argc, char **argv) {
    int optionChar;
    uint64_t numPuts, memtSize = 100000000;
    size_t keySize, valueSize;
    while ((optionChar = getopt (argc, argv, ":n:k:v:m:")) != -1) {
        switch (optionChar) {
            case 'n': numPuts = atoi (optarg); break;
            case 'k': keySize = atoi (optarg); break;
            case 'v': valueSize = atoi (optarg); break;
            case 'm': memtSize = atoi (optarg); break;
