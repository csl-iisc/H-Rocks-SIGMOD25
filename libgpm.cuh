...
static __device__ void gpm_flush(const void *addr, size_t len)
{
    ...
    volatile GPM_WORD *c = (GPM_WORD *)((BYTE *)addr + i);    
    // Store elements at word granularity
    for(; i + sizeof(GPM_WORD) <= len && 
        ((size_t)addr % sizeof(GPM_WORD)) == 0
        ; i += sizeof(GPM_WORD), c += 1) {
        // Store the value in addr to a volatile copy 
        // of itself to guarantee cache flush
        PMEM_WRITE_OP( (*c) = *((GPM_WORD *)addr + (i / sizeof(GPM_WORD))) , sizeof(GPM_WORD) )
    }
...
static __device__ cudaError_t gpm_memcpy_nodrain(void *gpmdest, const void *src, size_t len, cudaMemcpyKind kind)
{
    ...
    volatile GPM_WORD *c = (GPM_WORD *)((BYTE *)gpmdest + i);
    // Copy elements at word granularity
    for(; i + sizeof(GPM_WORD) <= len && 
        ((size_t)gpmdest % sizeof(GPM_WORD)) == 0 && 
        ((size_t)src % sizeof(GPM_WORD)) == 0
        ; i += sizeof(GPM_WORD), c += 1) {
        // Store the value to a volatile copy to guarantee cache flush
        PMEM_WRITE_OP( (*c) = *((GPM_WORD *)src + (i / sizeof(GPM_WORD))) , sizeof(GPM_WORD) )
    }
...
static __device__ cudaError_t gpm_memset_nodrain(void *gpmdest, unsigned char value, size_t len)
{
    ...
    if(value == 0)
    {
        ...
        ; i += sizeof(GPM_WORD), c += 1) {
            // Store the value to a volatile copy to guarantee cache flush
            PMEM_WRITE_OP( (*c) = value , sizeof(GPM_WORD) )
        }
    }
...
static __device__ void vol_memcpy(void *dest, const void *src, size_t len)
{
    ...
    volatile GPM_WORD *c = (GPM_WORD *)((BYTE *)dest + i);
