...
__device__
uint64_t lookupKey(char* key, GMemtable** table, char* value, uint64_t opID, int batchID) {
    uint64_t start = 0;
    ...
    while (start <= end) {
        ...
        if (cmp == 0) {
            ...
            // return mid; 
            return handleCmpEqual(key, table, value, opID, mid, batchID); // TODO: fix this
        } else if (cmp < 0) {
            end = mid - 1;
        } else {
            start = mid + 1;
...
__device__ void flush(SharedBuffer* sharedBuffer) {
...
void binarySearch(GpuReadBatch* batch, GMemtable** activeTable, GMemtable** immutableTable, uint64_t numKeys, int keyLength, 
    ...
    for(uint64_t idx = threadID; idx < numKeys; idx += blockDim.x * gridDim.x) {
        ...
        index = lookupKey(key, activeTable, gValuePointersArr[idx], readOpID, batchID); // sets the index and value

        if (gValuePointersArr[idx] != NULL) {
            return;
        }

        for (int i = 0; i < numTables - 1; i++) {
            index = lookupKey(key, &immutableTable[i], gValuePointersArr[idx], readOpID, batchID);
            if (gValuePointersArr[idx] != NULL) {
                return;
            }
        }        
...
__global__
void binarySearchNoSharedBufferTest(GpuReadBatch* batch, GMemtable** activeTable, GMemtable** immutableTable, uint64_t numKeys, int keyLength, 
        int numTables, char** gValuePointersArr, BlockCache* blockCache, struct NotFoundBuffer* notFoundBuffer) {
    uint64_t threadID = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    int batchID = batch->batchID; // This can be across batches also and not just within the same batchf
    for(uint64_t idx = threadID; idx < numKeys; idx += blockDim.x * gridDim.x) {
        long long int index = -1;
        // Lookup the key in the activeTable and then all the immutableTables 
        // If the key is found in any of the tables, then return the value
        ...
        gValuePointersArr[idx] = lookupBlockCache(key, blockCache);
        // copy key to keyNotFoundBuffer
        // atomic increment numKeyNotFound
        if (index == -1) {
            atomicAdd(notFoundBuffer->numNotFoundKeys, 1);
            notFoundBuffer->notFoundKeysBitMap[idx] = true;
        }
    }
}
...
void GpuGets::search() {
    ...
    timer->startTimer("gpuSearch", readBatch->batchID);
    ...
    std::cout << "Number of reads: " << gReadBatch->numReads << " keyLength: " << gReadBatch->keyLength << "\n";
    ...
    // Assuming readBatch->keys, readBatch->opIDArr, and readBatch->outputValuesPtr are properly allocated and initialized vectors or arrays
    char* keys = readBatch->keys.data();
    uint64_t* opIDArr = readBatch->opIDArr.data();
    char** outputValuesPtr;

    // Register the pointers with CUDA
    cudaHostRegister(keys, readBatch->numReads * readBatch->keyLength, cudaHostRegisterDefault);
    cudaHostRegister(opIDArr, readBatch->numReads * sizeof(uint64_t), cudaHostRegisterDefault);
    // Uncomment if outputValuesPtr needs to be registered
    // cudaHostRegister(outputValuesPtr, readBatch->numReads * sizeof(char*), cudaHostRegisterDefault);
    ...
    cudaMemset(gReadBatch->outputValuesPtr, 0, readBatch->numReads * sizeof(char*));
    ...
    binarySearchNoSharedBufferTest<<<NBLKS, NTHREADS_PER_BLK>>>(gReadBatch, activeTable, immutableTable, numKeys, keyLength, numMemtables, 
        gReadBatch->outputValuesPtr, blockCache, notFoundBuffer);
    
    
    cudaDeviceSynchronize();
    timer->stopTimer("gpuSearch", readBatch->batchID);
    // std::cout << "numKeysNotFound: " << *notFoundBuffer->numNotFoundKeys << "\n";
    std::cout << "numKeysNotFound: " << readBatch->numReads * 0.8 << "\n";
