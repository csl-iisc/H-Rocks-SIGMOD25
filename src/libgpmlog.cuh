...
{    
    ...
    for(; i < size && temp_head < tail;) {
        ...
        int sz = 1;
        if(size - i >= GPM_WORD_SIZE - (int)(temp_head % GPM_WORD_SIZE))
            sz = GPM_WORD_SIZE - (int)(temp_head % GPM_WORD_SIZE);
        
        PMEM_READ_OP( gpm_memcpy_nodrain((char *)plog->start + temp_head, (char *)var + i, sz, cudaMemcpyDeviceToDevice) , sz )
        
        temp_head += (size_t)sz;
        i += sz;
        // Reached end of GPM_WORD_SIZE byte segment
        // move to next segment
        if(temp_head % GPM_WORD_SIZE == 0) {
            // next address = current address + BLOCK_SIZE - GPM_WORD_SIZE
            temp_head += BLOCK_SIZE - GPM_WORD_SIZE;
        }
    }
    PMEM_READ_OP( , 8 )
...
{    
    ...
    if(head / BLOCK_SIZE * GPM_WORD_SIZE + head % GPM_WORD_SIZE < size) {
        size = head / BLOCK_SIZE * GPM_WORD_SIZE + head % GPM_WORD_SIZE;
    }
    ...
    for(;j > 0;) {
        ...
        if(i % GPM_WORD_SIZE == 0) {
            // next address = current address - BLOCK_SIZE + GPM_WORD_SIZE
            i -= BLOCK_SIZE - GPM_WORD_SIZE;
        }
        
        // Read up to a word at a time
        size_t sz = 1;
        if(i % GPM_WORD_SIZE == 0 && j >= GPM_WORD_SIZE)
            sz = GPM_WORD_SIZE;
        else if(j >= i % GPM_WORD_SIZE)
            sz = i % GPM_WORD_SIZE;
        else
            sz = j;
...
{    
    ...
    int new_head = head / BLOCK_SIZE * GPM_WORD_SIZE + head % GPM_WORD_SIZE - size;
    // Move head to appropriate position
    int offset = (tid < WARP_SIZE ? 0 : plog->plog->tail[tid - WARP_SIZE]) + (tid % WARP_SIZE) * GPM_WORD_SIZE;
    new_head = new_head / GPM_WORD_SIZE * BLOCK_SIZE + new_head % GPM_WORD_SIZE + offset;
    // Drain if necessary
    if(PMEMLOG_UNSTRICT & plog->plog->flags) {   
...
static __device__ void gpmlog_clear_managed(gpmlog *plog, int partition)
{
    ...
    if(partition != -1)
        tid = partition;
    PMEM_READ_OP( , sizeof(size_t) )
    size_t head = (tid % WARP_SIZE) * GPM_WORD_SIZE + (tid < WARP_SIZE ? 0 : plog->plog->tail[tid - WARP_SIZE]);
...
static __global__ void setupPartitionsManaged(size_t *head, size_t *tail, int blocks, int threads, size_t len)
{
    ...
    head[blk * threads + thd] = (regions_per_warp * BLOCK_SIZE) * (id / 32) + (id % 32) * GPM_WORD_SIZE;
...
static __device__ int gpmlog_is_empty(gpmlog *plog, int partition = -1)
{
    ...
    if(plog->plog->flags & PMEMLOG_UNMANAGED) {
        ...
        if(head / BLOCK_SIZE * GPM_WORD_SIZE + head % GPM_WORD_SIZE <= 0) {
            empty = true;
        }
    }
    return empty;
