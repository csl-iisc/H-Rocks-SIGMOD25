...
GpuGets::GpuGets(ReadSubBatch* readBatch, GMemtable** activeTable, GMemtable** immutableMemtable, BlockCache* blockCache, 
        NotFoundBuffer* notFoundBuffer, DbTimer* timer) :  readBatch(readBatch), activeTable(activeTable), immutableTable(immutableTable), 
        blockCache(blockCache), notFoundBuffer(notFoundBuffer), timer(timer) {
    ...
    std::cout << "Print num reads: " << numKeys << "\n";
