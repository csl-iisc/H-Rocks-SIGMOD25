...
void DbTimer::startTimer(const std::string& category, int instanceId) {
...
void DbTimer::stopTimer(const std::string& category, int instanceId) {
    std::string key = category + std::to_string(instanceId);
...
double DbTimer::getTotalTime(const std::string& category) {
    double total = 0.0;
...
double DbTimer::getAverageTime(const std::string& category) {
    double total = 0.0;
    int count = 0;
    for (const auto& item : records) {
        if (item.first.find(category) == 0) {  // Check if the category name is at the start of the key
...
double DbTimer::getThroughput(const std::string& category) {
    double total = 0.0;
    for (const auto& item : records) {
        if (item.first.find(category) == 0) {  // Check if the category name is at the start of the key
            for (const auto& time : item.second) {
